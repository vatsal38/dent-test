...
function partnershipPanelOrgLine(
...
export default function PartnershipsPage() {
  ...
  const [partnershipTypes, setPartnershipTypes] = useState<
    Array<{ partnershipType: string; label: string; count: number }>
  >([]);
  ...
  const loadData = useCallback(
    async (showLoading = true) => {
      ...
      try {
        ...
        setError(
          err instanceof Error ? err.message : "Failed to load partnerships",
        );
      } finally {
  ...
  const getTypeLabel = (value: string) =>
    partnershipTypes.find((t) => t.partnershipType === value)?.label ??
  ...
  const allRoleOptions = useMemo(() => {
    const list = (
      partnershipTypes.length > 0
        ? partnershipTypes
        : Object.entries(PARTNERSHIP_TYPE_LABELS).map(
            ([partnershipType, label]) => ({
              partnershipType,
              label,
              count: 0,
            }),
      ...
      .sort((a, b) =>
        (a.label || a.partnershipType).localeCompare(
          b.label || b.partnershipType,
        ),
      );
    return list;
  }, [partnershipTypes]);
  ...
  return (
    <div className="relative flex flex-col md:flex-row h-screen min-h-0 bg-white overflow-hidden">
      {successToast && (
      ...
      {/* Main Content */}
      <div
        className={`flex-1 min-w-0 min-h-0 overflow-auto transition-all ${selectedPartnership ? "hidden md:block md:flex-none md:w-2/3" : "w-full"}`}
      >
        <div>
          {/* Header */}
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-6">
            <div>
              ...
              <p className="text-gray-600">
                Manage all partnership stages and activities in one view.
              </p>
            </div>
          ...
          <div className="mb-6">
            <label className="block text-sm font-medium text-gray-700 mb-2">
              Filter by Role / Potential Roles
            </label>
                ...
                >
                  <span>
                    {selectedTypes.length > 0
                      ? `Roles (${selectedTypes.length} selected)`
                      : "Select roles"}
                  </span>
                  <svg
                          ...
                          return (
                            ...
                            >
                              <div className="flex items-center gap-3 min-w-0">
                                <span
                                  className={`w-4 h-4 rounded border flex items-center justify-center ${checked ? "bg-blue-600 border-blue-600" : "border-gray-300 bg-white"}`}
                                >
                                  {checked && (
                                    <svg
          ...
          {view === "kanban" && data.columns && (
            <div className="flex gap-6 overflow-x-auto pb-4 -mx-6 px-6">
              {refreshing
                ? Array.from({
                    length: Math.min(5, data.columns.length || 5),
                  }).map((_, idx) => (
                    <div
                      key={idx}
                              ...
                              {filteredPartnerships.length === 0 ? (
                                ...
                                filteredPartnerships.map((partnership) => {
                                  ...
                                  const isStuck =
                                    ...
                                    daysSinceUpdate >= PARTNERSHIP_STALL_DAYS;
                                  ...
                                  const orgLine =
                                    partnershipListOrgLine(partnership);
                                    ...
                                    >
                                      <div className="flex items-start justify-between mb-1">
                                        <h4 className="font-semibold text-gray-900 text-sm flex-1 pr-2">
                                          {partnershipListPrimaryTitle(
                                            partnership,
                                          )}
                                        </h4>
                                        <span
                                          className={`px-2 py-0.5 rounded text-xs font-medium border shrink-0 ${statusColor}`}
                                        >
                                          {status}
                                        </span>
                                      </div>
                                      ...
                                      {(partnership.tFocus ||
                                        partnership.revenueCommitment != null ||
                                        partnership.estimatedQuote != null ||
                                        partnership.finalQuote != null) && (
                                      ...
                                      <div className="space-y-1 mb-2">
                                        <p
                                          className={`text-xs ${isStuck ? "text-red-700 font-semibold" : "text-gray-500"}`}
                                        >
                                          {lastUpdateLabel(daysSinceUpdate)}
                                          {partnership.daysSinceContact !==
                                            null && (
                        ...
                        <div className="col-span-4">
                          ...
                          {(partnership.tFocus ||
                            ...
                            <p className="text-[11px] text-gray-600 mt-1">
                              ...
                              {formatMoney(partnership.revenueCommitment)}
                              {" • "}
                              <span className="font-medium">Est:</span>{" "}
                              {formatMoney(partnership.estimatedQuote)}
                              {" • "}
                              <span className="font-medium">Final:</span>{" "}
                              {formatMoney(partnership.finalQuote)}
                            </p>
                          )}
                        </div>
                        <div className="col-span-2">
...
function PartnershipPanel({
  ...
  const [partnership, setPartnership] = useState<PartnershipDetail | null>(
    null,
  );
  ...
  const [revenueCommitmentInput, setRevenueCommitmentInput] =
    useState<string>("");
  ...
  const loadPartnership = useCallback(async () => {
    ...
    try {
      ...
      setPartnership(data);
      const roles = Array.isArray(data.partnershipType)
      ...
      setRevenueCommitmentInput(
        data.revenueCommitment != null ? String(data.revenueCommitment) : "",
      );
      setEstimatedQuoteInput(
        data.estimatedQuote != null ? String(data.estimatedQuote) : "",
      );
      setFinalQuoteInput(
        data.finalQuote != null ? String(data.finalQuote) : "",
      );
    } catch (err) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        setOrgUsers([]);
        setOrgUsersError(
          e instanceof Error ? e.message : "Could not load team members",
        );
      } finally {
  ...
  async function handleInternalOperatorChange(nextUserId: string) {
    ...
    const currentId = prev.internalOperator?.userId
      ? String(prev.internalOperator.userId)
      : "";
    ...
    try {
      ...
      onSuccessToast?.(
        normalized
          ? "Assigned internal operator."
          : "Cleared internal operator.",
      );
    } catch (err) {
  ...
  const daysSinceContact = partnership.lastContactAt
    ? Math.floor(
        (new Date().getTime() - new Date(partnership.lastContactAt).getTime()) /
          (1000 * 60 * 60 * 24),
      )
    : null;

  const needsAttention =
    partnership.stage === "mou_sent" &&
  ...
  return (
    <div className="w-full bg-white flex flex-col h-full min-h-0 overflow-y-auto shrink-0">
      <div className="p-6 border-b border-gray-200 bg-white">
        <div className="flex items-start justify-between mb-4">
          <div className="flex-1 min-w-0">
            <h2 className="text-xl font-bold text-gray-900 truncate">
              {partnershipPanelPrimaryTitle(partnership)}
            </h2>
            {panelOrgLine && (
            ...
            <div className="mt-4">
              ...
              <p className="mt-1.5 text-[11px] text-gray-500">
                App only — not synced to Airtable.
              </p>
              {orgUsersError && (
          ...
          <div className="p-3 bg-red-50 border border-red-200 rounded-lg mb-4">
            <div className="flex items-center gap-2">
              ...
              <p className="text-sm font-medium text-red-900">
                This partnership needs immediate attention
              </p>
            </div>
          </div>
        )}
      ...
      <div className="p-6 space-y-6">
                      ...
                      <div className="flex-1">
                        ...
                        {contact.email && (
                          <a
                            ...
                            className="text-sm text-[#3b82f6] hover:underline mt-1 block"
                          >
                            {contact.email}
                          </a>
                        )}
          ...
          <div className="space-y-3 mb-4">
            ...
            {(() => {
              const hasAirtable =
                ...
                partnership.contacts.some(
                  (c) => (c.notes || "").trim().length > 0,
                ) ||
                !!partnership.airtableRecordUrl;
              const noteActivities = partnership.activities.filter(
                (a) => a.type === "note",
              );
              ...
              partnership.activities.map((activity) => {
                      ...
                      return (
                        ...
                        >
                          <path
                            ...
                            strokeLinejoin="round"
                            strokeWidth={2}
                            d="M17 20h5V9H2v11h5m10 0v-4a3 3 0 00-3-3H10a3 3 0 00-3 3v4m10 0H7m8-13a3 3 0 11-6 0 3 3 0 016 0z"
                          />
                        </svg>
                      );
                    case "contact_added":
                      return (
                        ...
                        >
                          <path
                            ...
                            strokeLinejoin="round"
                            strokeWidth={2}
                            d="M18 9v3m0 0v3m0-3h3m-3 0h-3M5 20a7 7 0 1111.95-4.95M15 7a3 3 0 11-6 0 3 3 0 016 0z"
                          />
                        </svg>
                      );
                      ...
                      return (
                        ...
                        >
                          <path
                            ...
                            strokeLinejoin="round"
                            strokeWidth={2}
                            d="M4 4v5h.582m14.836 2A8 8 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-14.836-2M15 15h.01"
                          />
                        </svg>
                      );
                    default:
                ...
                const getActivityLabel = () => {
                  switch (activity.type) {
                    ...
                    case "airtable_imported":
                      return "Airtable imported";
                    case "airtable_updated":
                      return "Airtable updated";
                    default:
                ...
                return (
                  <div
                    key={activity.id}
                    className={`flex items-start gap-3 p-3 rounded-lg border ${styles.row}`}
                  >
                    <div
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm text-gray-900">
                        {activity.content || getActivityLabel()}
                      </p>
                      <p className="text-xs text-gray-500 mt-1">
        ...
        {showEmailComposer && partnership && (
          <EmailComposer
            ...
            to={
              partnership.contacts?.find((c) => c.isPrimary)?.email ||
              partnership.contacts?.[0]?.email ||
              ""
            }
