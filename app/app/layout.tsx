  ...
  if (status.syncErrorMessage) {
    return (
      <div className="flex items-center gap-2 px-3 py-1.5 bg-yellow-50 border border-yellow-200 rounded-full">
        ...
        >
          <HiOutlineRefresh
            className={`w-4 h-4 shrink-0 ${syncing ? "animate-spin" : ""}`}
          />
        </button>
      </div>
    );
...
export default function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <aside
        className={`w-64 shrink-0 flex flex-col bg-white border-r border-gray-200 fixed inset-y-0 left-0 z-50 transform transition-transform duration-200 ease-out md:translate-x-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          >
            ...
            <span className="px-1.5 py-0.5 text-xs font-bold rounded bg-orange-500 text-white">
              2026
            </span>
          </div>
          <nav className="p-3 pt-0 space-y-0.5">
            ...
            <div className="pt-1">
              ...
              >
                ...
                <svg
                  className={`w-4 h-4 shrink-0 transition-transform ${bobMoreExpanded ? "rotate-180" : ""}`}
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
