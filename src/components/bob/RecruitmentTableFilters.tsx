...
export type { RecruitmentTableFilterState } from "@/lib/bobRecruitmentFilters";
export {
  EMPTY_RECRUITMENT_FILTERS,
  countActiveRecruitmentFilters,
  countDrawerRecruitmentFilters,
...
function ConditionRow({
  ...
  condition: RecruitmentFilterCondition;
  index: number;
  match: "and" | "or";
    ...
    <div className="flex flex-wrap items-start gap-2">
      ...
      <select
        ...
        onChange={(e) =>
          onChange({
            ...condition,
            operator: e.target.value as RecruitmentFilterCondition["operator"],
...
function AirtableFilterBuilder({
  ...
  function updateCondition(id: string, next: RecruitmentFilterCondition) {
    onChange({
      ...draft,
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-[11px] text-gray-500 leading-relaxed">
        ...
        Yes/No for transfer, roster, and sync.
      </p>
    </div>
  );
}
...
export function RecruitmentRecordsToolbar({
  filters,
  facets,
  facetsLoading,
  schema,
  ...
  filters: RecruitmentTableFilterState;
  facets: BobRecruitmentFacetsResponse | null;
  facetsLoading?: boolean;
  schema: BobRosterSchemaField[] | null;
  programOptions?: Array<{ id: string; label: string }>;
  ...
  onApplyDrawerFilters: (drawer: Omit<RecruitmentTableFilterState, "search">) => void;
  onClearDrawerFilters: () => void;
}) {
  const [draft, setDraft] = useState(filters);
  const drawerActive = countDrawerRecruitmentFilters(filters);
  const fieldCatalog = useMemo(() => buildFilterFieldCatalog(schema), [schema]);
    ...
    <>
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-4">
        <div className="relative flex-1 min-w-0">
          ...
          <input
            type="search"
            placeholder="Search name, email, phone, school, status, notes…"
...
/** @deprecated Use RecruitmentRecordsToolbar */
export function RecruitmentTableFilters(props: {
  filters: RecruitmentTableFilterState;
  facets: BobRecruitmentFacetsResponse | null;
  facetsLoading?: boolean;
  schema?: BobRosterSchemaField[] | null;
