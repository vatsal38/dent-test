...
export async function bootstrap(): Promise<BootstrapResponse> {
  ...
  if (!response.ok) {
    const error = new Error(data.error || "Bootstrap failed") as Error & {
      code?: string;
    };
...
export async function getPartnerships(options?: {
  ...
  return apiRequest<PartnershipsListResponse>(
    `/api/education/partnerships${qs ? `?${qs}` : ""}`,
  );
}
...
export async function getPartnershipDetails(
  ...
  return apiRequest<PartnershipDetail>(
    `/api/education/partnerships/${partnershipId}`,
  );
}
  ...
  if (!response.ok) {
    ...
    if (response.status === 409 && responseData.duplicates) {
      const error = new Error(
        responseData.message || "Duplicate found",
      ) as Error & { status?: number; data?: DuplicateErrorResponse };
      error.status = 409;
...
export async function createPartnershipForExistingOrg(
  ...
  return apiRequest<CreatePartnershipResponse>(
    `/api/education/partnerships/existing/${partnerOrgId}`,
    {
      method: "POST",
      body: JSON.stringify({
        ...data,
...
export async function getPartnershipTotals(options?: {
  assignedTo?: string;
}): Promise<PartnershipTotalsResponse> {
  const params = new URLSearchParams();
  if (options?.assignedTo) params.set("assignedTo", options.assignedTo);
  const qs = params.toString();
  return apiRequest<PartnershipTotalsResponse>(
    `/api/education/partnerships/totals${qs ? `?${qs}` : ""}`,
  );
}
...
export async function getGmailThreads(options?: {
  ...
  if (options?.partnershipContactsOnly)
    params.set("partnershipContactsOnly", options.partnershipContactsOnly);
  const qs = params.toString();
  return apiRequest<GmailThreadsResponse>(
    `/api/education/inbox/threads${qs ? `?${qs}` : ""}`,
  );
}
...
export async function createPartnerFromEmail(
  ...
  partnership: { id: string; stage: string };
}> {
...
export async function searchAirtable(
  query: string,
  options?: { partnershipType?: string },
): Promise<{ records: AirtableRecord[] }> {
  const params = new URLSearchParams();
  params.set("search", query);
  if (options?.partnershipType)
    params.set("partnershipType", options.partnershipType);
  return apiRequest<{ records: AirtableRecord[] }>(
    `/api/integrations/airtable/search?${params.toString()}`,
  );
}

export async function importAirtableRecord(
  recordId: string,
  options?: { partnershipType?: string; tags?: string[] },
): Promise<{ partnershipId: string; success: boolean }> {
  return apiRequest<{ partnershipId: string; success: boolean }>(
    "/api/education/partnerships/import-airtable",
    {
      method: "POST",
      body: JSON.stringify({
        recordId,
...
export async function getRunTemplates(): Promise<{ templates: RunTemplate[] }> {
  return apiRequest<{ templates: RunTemplate[] }>(
    "/api/education/runs/templates",
  );
}
...
// ============================================
// BOB — re-exported from @/platform/api/bob
// ============================================

export * from "@/platform/api/bob";
