...
import { resolveBobAirtableRecordLabels } from "@/platform/api/bob/airtable";
import {
...
function renderValueNode(
  value: unknown,
  opts?: {
    resolveLinkedId?: (id: string) => { label: string; href?: string } | null;
  },
) {
...
function groupForFieldName(name: string): FieldGroupKey {
  ...
  if (
    /(email|e-mail|phone|mobile|cell|whatsapp|contact|address|city|location|linkedin|instagram)/.test(
      n,
    )
  )
    return "Contact";
  if (
    /(status|stage|pipeline|funnel|interview|round|result|decision|offer|enroll)/.test(
      n,
    )
  )
    return "Pipeline";
  if (/(source|lead source|referral|campaign|utm|how did you hear)/.test(n))
    return "Source";
  if (/(date|time|schedule|slot|call|meeting|calendar)/.test(n))
    return "Scheduling";
...
export function RecruitmentDetailPage() {
      ...
      try {
        ...
        for (let i = 0; i < 20; i++) {
          ...
          for (const s of resp.students || []) {
            const rid = (s as any).airtableRecordId as
              | string
              | null
              | undefined;
            if (!rid) continue;
  ...
  const studentLabelByRid = useMemo(() => {
    ...
    for (const [rid, s] of Object.entries(studentByAirtableId))
      m.set(rid, s.label);
    return m;
  }, [studentByAirtableId]);
  ...
  const highlight = useMemo(() => {
    const get = (name: string) =>
      normalizedDraftFields.get(normalizeKey(name))?.value ?? "";
    ...
    <div className="p-6 sm:p-8 max-w-[1100px] mx-auto">
      <div className="mb-5">
        ...
        <div className="mt-4 rounded-lg border border-gray-200 bg-gradient-to-b from-gray-50 to-white p-4">
          ...
          {recruitmentSchema?.programOptions &&
          recruitmentSchema.programOptions.length > 0 ? (
            <div>
              <label className="block text-xs font-semibold text-gray-700 mb-2">
                Programs{" "}
                {recruitmentSchema.programsFieldName
                  ? `(${recruitmentSchema.programsFieldName})`
                  : ""}
              </label>
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <p className="text-xs text-gray-500">
                  Saves to intake track-choice fields (Top / Second / Other)
                  and, after transfer,{" "}
                  <span className="font-medium">Programs</span> on Students
                  &amp; Alums. Linking{" "}
                  <span className="font-medium">Your Name</span> on the intake
                  row fills &quot;Programs (from Your Name)&quot; in Airtable.
                </p>
              </div>
            </div>
          ) : (
            <p className="text-xs text-gray-500">
              No Programs table linked in Airtable schema, or options could not
              be loaded.
            </p>
          )}
        </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
              <div>
                <div className="text-sm font-semibold text-amber-900">
                  You’re editing this recruitment record
                </div>
                <div className="text-xs text-amber-800 mt-0.5">
                  Update fields below, then press{" "}
              ...
              <div className="text-xs text-amber-800">
                Tip: search by field name or value.
              </div>
            </div>
          </div>
        )}
                ...
                {fields.length === 0 ? (
                  ...
                  fields.map((f) => {
                    ...
                    return (
                      <div
                        key={f.name}
                        className="py-3 grid grid-cols-1 sm:grid-cols-12 gap-2 border-b border-gray-100"
                      >
                          ...
                          {editing ? (
                            (() => {
                              const isLinkedField =
                                ...
                                linkedRecordSummaryFromRaw(String(raw))
                                  .linkedCount > 0;
                              ...
                              return (
                                <div className="rounded-lg border border-gray-200 bg-white px-3 py-2">
                                  ...
                                  <MultiSelectDropdown
                                    ...
                                    selected={extractAirtableRecordIdsFromRaw(
                                      String(raw),
                                    )}
                                    onChange={(selected) => {
                                      setDraftFields((d) => ({
                                  ...
                                  <div className="mt-2 text-xs text-gray-600">
                                    Selected:{" "}
                                    {(() => {
                                      const parsed = parseArrayFromString(
                                        String(raw),
                                      );
                                      const ids = parsed
                                        ? parsed
                        ...
                        <div className="sm:col-span-1 flex sm:justify-end">
                          {!editing ? (
                            <button
                              type="button"
                              onClick={() =>
                                void copyToClipboard(String(raw ?? ""))
                              }
